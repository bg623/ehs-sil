...
export function checkCase(s){const issues=[];const warn=(step,text)=>issues.push({step,text});
 ...
 if(s.plan.depth==='待决定'||!s.plan.lead.trim()||!s.plan.scope.trim())warn(1,'补充调查深度、负责人及范围。');if(!s.plan.reporting.trim())warn(1,'记录法定报告义务的人工核对情况；本工具不判断是否需要报告。');
 const depths=['快速调查','标准调查','深度调查'];if(s.plan.depth!=='待决定'&&depths.indexOf(s.plan.depth)<depths.indexOf(recommend(s).depth))warn(1,'人工选定深度低于建议，请由调查负责人复核并记录理由；本工具不批准降低调查深度。');
...
export function parseBackup(text){if(new TextEncoder().encode(text).length>MAX_BACKUP_BYTES)throw Error('备份超过 16 MB。');const raw=JSON.parse(text);if(raw?.version!==VERSION)throw Error('不是本工具当前版本的备份。');const s=createCase();
 ...
 const references=COLLECTIONS.flatMap(g=>s[g].flatMap(r=>DEFINITIONS[g].filter(d=>d.type==='refs').flatMap(d=>r[d.key])));
 for(const g of COLLECTIONS){s.next[g]=Math.max(s.next[g],...references.filter(id=>id.startsWith(PREFIX[g])).map(id=>Number(id.slice(1))+1));if(s.next[g]>=1000000)throw Error('备份编号已超出支持范围。');}
 return s;
}
