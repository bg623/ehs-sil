...
export function buildTrainingWorkbook(ExcelJS,result,data,state,date=new Date().toISOString().slice(0,10)){
  ...
  detail.addRow(['岗位','培训主题','代码','要求分类','优先级','匹配原因','依据名称','依据层级','依据状态','实施日期','触发条件','频次','最低学时','目标能力','培训方式','效果验证','记录证据','企业复核','规则ID']);
  for(const item of result.requirements){
    detail.addRow([
      item.role_name,item.topic.title_zh,statusCode(item.requirement_level),levelLabel[item.requirement_level],item.priority,
      ...
      join(item.sources.map(source=>source.effective_date)),join(item.training_triggers),join(item.frequencies),join(item.minimum_durations),
  ...
  info.addRows([
    ['EHS-SIL 培训矩阵生成器',`V${result.version}`],['生成日期',date],['法规版本核验日期',data.verified_at],['适用基线',data.jurisdiction],['行业',industry],
    ['选择岗位',result.roles.map(x=>x.name).join('、')],['选择属性/风险',state.risks.map(id=>data.risks.find(x=>x.id===id)?.name).filter(Boolean).join('、')],
