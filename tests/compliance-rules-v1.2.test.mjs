...
const noise=E.identify(regulations,{province:"江苏省",enterpriseTypes:["机械制造企业"],riskTags:["产生工业噪声"]},{rules,today:"2026-08-03"});
assert.ok(noise.some(r=>r.id==="REG-064"&&r.applicability==="明确适用"),"industrial-noise profile must match the verified noise law");
assert.ok(!noise.some(r=>r.name==="环境噪声污染防治法"),"superseded noise-law title must not be returned");
const wasteReview=E.identify(regulations,{province:"江苏省",enterpriseTypes:["机械制造企业"],riskTags:["产生一般固体废物或危险废物"]},{rules,today:"2026-08-03"});
assert.ok(wasteReview.some(r=>r.id==="REG-ENV-002"&&r.applicability==="建议复核"),"coarse waste profile must request review");
const wasteDirect=E.identify(regulations,{province:"江苏省",enterpriseTypes:["机械制造企业"],hazardousWasteActivities:["危废产生"]},{rules,today:"2026-08-03"});
assert.ok(wasteDirect.some(r=>r.id==="REG-ENV-002"&&r.applicability==="明确适用"),"hazardous-waste activity must match the 2025 catalogue");
const groundwater=E.identify(regulations,{province:"山东省",enterpriseTypes:["一般化工企业"],industries:["化学原料和化学制品制造"]},{rules,today:"2026-08-03"});
assert.ok(groundwater.some(r=>r.id==="REG-ENV-001"&&r.applicability==="明确适用"),"chemical-manufacturing profile must surface groundwater requirements");
console.log(JSON.stringify({status:"PASS",cases:cases.length,shandong:sd.length,jiangsu:js.length,coarseSpecialOperation:coarse.length,verifiedEnvironment:3}));
