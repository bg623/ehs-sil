...
assert.match(html,/前 6 条结果/);assert.match(html,/会员导出完整 Excel/);assert.match(app,/FREE_PREVIEW_LIMIT=6/);assert.match(app,/preview\(filtered\(\)\)/);
...
const authCode=read("js/auth.js"),responses=[{ok:true,active:true,capabilities:["compliance_excel_export"]},{ok:true,active:false,capabilities:[]}];let call=0;const storage={getItem(){return null},setItem(){},removeItem(){}};const sandbox={window:{addEventListener(){},dispatchEvent(){},localStorage:storage},document:{addEventListener(){},hidden:false},localStorage:storage,location:{reload(){},pathname:"/tools/compliance-identification.html",search:""},URLSearchParams,CustomEvent:class{constructor(type,init){this.type=type;this.detail=init.detail}},console,setTimeout,fetch:async()=>({ok:true,json:async()=>responses[call++],status:200})};sandbox.window.window=sandbox.window;vm.runInNewContext(authCode,sandbox);assert.equal(await sandbox.window.EhsSilVip.hasCapability("compliance_excel_export",true),true);assert.equal(await sandbox.window.EhsSilVip.hasCapability("compliance_excel_export",true),false);
