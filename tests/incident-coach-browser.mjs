...
try{for(const width of [1440,390,320]){
 ...
 const xml=execFileSync('unzip',['-p',xlsx,'xl/worksheets/*.xml'],{maxBuffer:2000000}).toString();assert.doesNotMatch(xml,/<f[ >]/,'User input must remain a string, never an Excel formula');
 const strings=execFileSync('unzip',['-p',xlsx,'xl/sharedStrings.xml'],{maxBuffer:2000000}).toString();assert.match(strings,/=1\+1/);assert.match(strings,/分析假设/);assert.match(strings,/效果依据/);
 ...
 await page.locator('#clear-case').click();assert.equal(await page.locator('#event-title').inputValue(),'');await page.locator('#backup-file').setInputFiles(json);assert.equal(await page.locator('#event-title').inputValue(),payload);
 await page.locator('#backup-file').setInputFiles({name:'invalid.json',mimeType:'application/json',buffer:Buffer.from('{"version":"bad"}')});assert.match(await page.locator('#coach-status').innerText(),/无法恢复/);assert.equal(await page.locator('#event-title').inputValue(),payload);
