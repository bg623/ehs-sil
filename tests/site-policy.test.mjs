...
assert.match(practice, /href="tools\/moc-coach\.html" class="workbench-link">免费开始 MOC 变更管理/);
assert.match(practice, /href="tools\/training-matrix\.html" class="workbench-secondary-link" data-home-action="start-training">生成培训矩阵/);
assert.match(home, /<a href="tools\/training-matrix\.html" data-home-action="start-training"><span><b>岗位EHS培训矩阵<\/b>[\s\S]*?<em>免费 · 生成<\/em><\/a>/);
...
assert.match(read("tools/index.html"), /下方资料索引用于检索和内容预览，不提供原资料下载/);
assert.match(read("tools/index.html"), /公开模板可在对应页面下载/);
