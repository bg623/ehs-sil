...
const html=fs.readFileSync(new URL("../tools/compliance-identification.html",import.meta.url),"utf8");assert.match(html,/不构成法律意见/);assert.match(html,/建议复核/);assert.match(html,/databaseCount/);assert.match(html,/application\/ld\+json/);assert.match(html,/导出项目/);assert.match(html,/compliance-workflow\.js/);assert.match(html,/compliance-export\.js/);const regulations=fs.readFileSync(new URL("../tools/regulations.html",import.meta.url),"utf8");assert.match(regulations,/class="compliance-cta"/);assert.match(regulations,/aria-label="立即进行企业适用法规智能识别"/);assert.match(regulations,/免费在线工具/);
