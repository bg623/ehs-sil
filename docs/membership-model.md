...
| `admin` | 是 | 内部管理与测试；同时具备 `feedback_admin` |
...
- 前端统一展示 `checking`、`active`、`inactive`、`expired`、`revoked`、`error`，其中异常不等于未激活，未知原因不推断为过期；
- 激活接口成功后必须再次读取 `/session`，只有 Cookie 会话被服务端确认后才显示已激活；
- 同页初始读取合并为一个请求，跨标签页只广播“需要重新验证”，不广播激活码、Cookie、昵称或权益字段；
- `ehs_display_nickname_v1` 只是当前浏览器偏好，不是会员身份，不参与能力判断；成功退出或切换激活对象时清除；
- 返回工具地址只允许本站 `/tools/*.html`、`/index.html` 或 `/`，拒绝查询参数、外域、反斜杠及脚本协议；
- 免费工具不因会员状态验证失败而阻塞。

## 反馈管理能力

`feedback_admin` 只授予 `entitlement_source=admin`。管理页是否隐藏不构成保护；`GET/PATCH /api/admin/feedback` 每次均由 Worker 校验 HttpOnly 会话。普通会员、游客和无会话请求不能读取或修改反馈。
