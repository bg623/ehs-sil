...
- 当前稳定生产基线：`baab78d`，GitHub Pages 于 2026-08-29 21:05:35（Asia/Shanghai）构建完成；
- 有效观察窗口：2026-08-29 21:05:35 至 2026-09-12 21:05:35；正式复盘：2026-09-13；
- 2026-08-11 至 2026-08-19 为入口连续变化的过渡观察段；2026-08-19 至 2026-08-29 为公共导航升级前的停止窗口；两段均不与当前窗口合并计算；
...
- `glossary_view`：术语库完整数据加载成功；
- `glossary_search`、`glossary_no_result`：执行查询及无结果查询；
- `glossary_result_open`、`glossary_filter`、`glossary_confusion_open`：打开词条、使用筛选和打开易混辨析；
- `glossary_learning_start`、`glossary_learning_complete`：进入今日10词及完成10词标记；
- `glossary_download`：下载完整445词Excel；
- `glossary_related_tool_click`、`glossary_membership_click`：从术语详情进入相关工具或浏览工具箱资源。

查询事件只允许携带中文、英文、混合或无语言字符类型，是否精确匹配缩写，结果数量区间，分类ID和筛选类型。不得上传完整搜索词、术语释义、学习结果、浏览器存储内容或其他用户输入。自动化验收与内部测试不得计入真实用户使用。
...
- `feedback_open`：打开反馈表单；
- `feedback_submit_success`：服务端完成持久化并返回反馈编号；
- `feedback_submit_failed`：提交未得到成功回执；
- `activation_verified`：激活请求后再次读取会话，并确认权益生效；
- `membership_status_error`：会员状态因网络、超时或服务异常暂未确认。

事件 V2 只允许工具 ID、页面类型、入口枚举、反馈类型和有限错误分类。不得发送反馈正文、昵称、联系方式、会员标识、反馈编号、激活码、任意 URL 或错误正文。服务端接收成功率以 D1 中通过基本校验的唯一 `request_id` 为口径；百度统计失败事件只说明客户端未取得成功回执，两者不得混算。管理员关闭反馈不计作已解决。

导航观察口径（2026-08-29）：一级导航、下拉分类和直接入口发生变化，`content_to_tool`及页面访问必须以`baab78d`上线后会话重新建立基线；风险评估、事故调查、LFI、培训矩阵和法规识别继续按各自`tool_id`或页面路径分开统计。导航曝光、下拉展开或内部验收不等于工具采用；真实价值仍以进入工具、完成任务、导出及真实工作反馈判断。
