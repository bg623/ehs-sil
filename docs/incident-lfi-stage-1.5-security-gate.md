...
状态：阶段1.5附条件验收通过，执行1.5.1契约修正；不得自行进入2A-Sandbox

## 0. 闸门结论与范围

阶段1.5.1是对审计域、邀请实体、角色授予和阶段门槛的契约修正，不是完整LFI业务功能开发。

本阶段只允许：

- 使用虚构标识完善Schema契约、RBAC、三类审计、邀请和负面测试定义；
- 输出认证、租户解析、数据分级、威胁模型与数据生命周期设计；
- 运行Schema设计测试、静态链接和全站回归。
...
- 修改首页及JSA、会员、法规业务逻辑；
- 将本文件视为法律意见、数据出境批准、2A-Sandbox启动批准或2A-Pilot上线批准。
...
sequenceDiagram
    ...
    API->>DB: 创建独立invitation、拟授予角色与绑定invitation_id的哈希挑战
    API->>Mail: 发送一次性邀请链接
    Mail-->>Invitee: 邀请通知
    Invitee->>API: GET/POST 邀请挑战
    API->>DB: 校验哈希、用途、期限、未消费、尝试次数
    alt 合法
        API->>DB: 同一事务激活邀请、用户及成员、授予角色并消费挑战
        API->>DB: 分别写入最小化security/tenant审计事件
...
- 每次邀请必须创建独立`invitations`记录，绑定租户、规范化邮箱、邀请人、拟授予角色、期限和状态；
- `accept_invite`挑战必须绑定`invitation_id`，其他挑战不得绑定；
- 同一邮箱收到不同工厂邀请时，各邀请的ID、租户、角色、挑战、状态和事务完全隔离；
- 挑战明文只通过邮件发送，数据库只保存哈希；
- 挑战必须限制用途、有效期、尝试次数和单次消费；
- 邀请激活、membership激活、角色授予和挑战消费必须在同一事务中完成，任何一步失败全部回滚；
- 不在URL、日志或分析平台记录挑战明文；
- 邀请者只能授予自己有权管理的租户角色。
...
恢复不得自动启用`disabled`用户或成员；恢复成功必须撤销全部旧会话。恢复请求等尚未确定租户的事件进入`security_audit_logs`，不得伪造`tenant_id`。
...
- `data/incidents/tenant-audit-contract-v0.2.json`：租户业务操作，必须有真实`tenant_id`；
- `data/incidents/security-audit-contract-v0.2.json`：登录、邀请、恢复、限速，可没有租户；
- `data/incidents/platform-audit-contract-v0.2.json`：部署、support grant和break-glass，平台管理员不需要也不得因此成为租户成员。

`platform_audit_logs.tenant_reference_id`只用于相关性，不授予租户访问权。账号恢复、登录失败等未确定租户的安全事件不得伪造`tenant_id`。
...
白名单外事件和字段必须被审计写入层拒绝。Schema通过`audit_event_policies`及三类`BEFORE INSERT`触发器强制验证事件名、JSON形状和允许字段，不只是读取JSON配置。拒绝访问本身也记录结果与原因码，但不得记录攻击者提交的敏感载荷。

三类日志均有独立的禁止更新和禁止删除触发器；审计策略表本身也不可修改或删除。

## 8.1 角色授予矩阵

权威机器可读契约：`data/incidents/role-grant-matrix-v0.2.json`。

- 禁止自我授予任何角色；
- `tenant_admin`不得向自己或他人授予`site_leader`；
- 只有`site_leader`可以授予或撤销`ehs_manager`；
- `site_leader`和`ehs_manager`等高权限变更必须使用step-up更高强度确认；
- 禁止禁用、撤销或替换后导致租户不存在有效`site_leader`；
- 角色变更只在`tenant_audit_logs`记录角色枚举，不记录自由文本；
- `site_leader`更换使用原有效负责人高强度确认、再次明确确认、原子替换及“至少一个有效负责人”不变量；
- 初始`site_leader`只通过一次性`tenant_bootstrap_grants`开通：平台线下核验被邀请邮箱，生成短时哈希凭据，在单一事务中消费凭据、激活用户和membership、授予角色并分别写入平台/租户最小审计。该流程不可重复。
...
以下契约修正和设计测试完成后，仍须产品负责人明确批准，才能进入2A-Sandbox：

1. Schema更名为`schema-v0.2.sql`并可在本地SQLite完整创建；
2. 三类审计域分离、不可变，事件与字段白名单由存储层强制；
3. 正式邀请实体、挑战绑定和原子接收事务契约通过；
4. 角色授予矩阵、初始site leader流程和最后负责人不变量通过设计测试；
5. 测试口径明确为：“20项项目测试通过；16个API负面场景已定义，尚未执行完整API测试。”

2A-Sandbox仅允许本地SQLite/Miniflare、Mock邮件和虚构数据。不得连接生产资源、发送真实邮件或使用真实身份、事故、附件和健康数据。

### 11.2 2A-Pilot部署门槛

只有以下条件全部满足并经产品负责人明确确认，才能部署2A-Pilot：
...
9. 16个跨租户/越权负面用例在2A-Sandbox转为真实API集成测试并全部通过。
...
15. 产品负责人明确回复批准“部署2A-Pilot”；1.5.1契约提交或2A-Sandbox批准均不等于Pilot批准。

## 12. 阶段1.5.1完成定义

- Schema基线完成指定五项修订；
- 10类/16个负面测试场景形成机器可读清单；
- 邀请、角色授予、RBAC和三类审计契约可机器校验；
- 锚点目标真实存在且自动测试验证；
- 全站自动测试、静态链接和Schema语法测试通过；
- 测试报告明确区分项目测试与尚未执行的API负面测试；
- 仅提交设计与测试，不创建任何Cloudflare资源，不自行进入2A-Sandbox。
