...
const out = path.join(root, 'data/training-matrix/catalog-v0.3.json');
...
const roles = [
  ...
  ['fire_control_operator','消防控制室值班/操作人员','管理与专业负责人','承担消防控制室值班或操作职责，需核对专门培训及适用资格要求。'],
...
const personnelStatuses = [
  ['new_hire','新入职','触发新上岗人员岗前培训；必须同时选择实际岗位。'],
  ['transferred','转岗','触发与新岗位职责和风险相匹配的重新培训。'],
  ['returned','复岗','触发复岗前的岗位安全培训与能力确认。'],
  ['dispatched','劳务派遣人员','使用单位应纳入统一安全管理并开展岗位培训。'],
  ['intern','实习人员','接收单位应开展相应安全培训并提供必要劳动防护。']
].map(([id,name,description])=>({id,name,group:'人员状态',description}));
...
const riskTags = [
  ...
  ['fire_key_unit','属于消防安全重点单位','企业法定属性','影响消防重点岗位、培训与演练要求。'],
  ['public_gathering_place','属于公众聚集场所','企业法定属性','员工消防安全培训至少每半年一次。'],
...
const keyAttributeIds = new Set(['hazchem_high_risk_unit','hazchem_use','major_hazard','regulated_process','fire_key_unit','public_gathering_place','occupational_hazards','special_equipment','pollutant_permit','hazardous_waste','env_emergency']);
for (const tag of riskTags) {
  if (!keyAttributeIds.has(tag.id)) continue;
  tag.applicability_mode = 'tri_state';
  tag.diagnostic_question = tag.id === 'hazchem_high_risk_unit'
    ? '本单位是否属于煤矿、非煤矿山、危险化学品、烟花爆竹、金属冶炼等规定范围的生产经营单位？请按许可证、行业类别和实际业务性质确认；仅使用危险化学品不能自动判定为“适用”。'
    : `企业是否已依据许可、法定名录、评价报告或主管部门认定，确认“${tag.name}”适用？`;
}
...
const sources = [
  ...
  ['special_work_19','特种作业人员安全技术培训考核管理规定','应急管理部令第19号','第二十三、二十六至二十八条及现行特种作业目录','部门规章','应急管理部','https://www.mem.gov.cn/gk/zfxxgkpt/fdzdgknr/202512/t20251219_589179.shtml','2026-06-01','effective','证书有效期6年；届满需要继续从事特种作业的，按规定完成培训、考试并换证，不再沿用每3年复审表述。具体作业项目须按目录核对。'],
  ...
  ['fire_education_109','社会消防安全教育培训规定','公安部等九部门令第109号','第十四条','部门规章','国务院','https://www.gov.cn/flfg/2009-06/02/content_1329919.htm','2009-06-01','effective','单位应对新上岗和进入新岗位职工开展岗前消防培训，对在岗职工每年至少培训一次；培训与演练频次分开记录。'],
...
const topics = [
  ...
  T('fire_control_room','消防控制室值班与操作','管理与治理','掌握消防控制系统、报警处置、设施联动、值班记录和异常升级。',['专门培训','设备实操'],['理论与实操考核'],['资格/能力证明','值班授权'],'certified'),
  ...
  T('fire_key_all','消防安全重点单位员工年度培训','消防与应急','重点单位每名员工至少每年完成一次消防安全培训。',['消防培训'],['知识与实操考核'],['培训记录'],'practical'),
  T('fire_public_all','公众聚集场所员工半年度培训','消防与应急','公众聚集场所员工至少每半年完成一次消防安全培训，并掌握组织引导疏散技能。',['消防培训','疏散实操'],['知识与实操考核'],['培训记录'],'practical'),
...
let n = 0;
const rules = [];
const inferredBasisKind = r => {
  if (r.basis_kind) return r.basis_kind;
  if (r.competence_level === 'certified') return 'statutory_qualification';
  if (r.requirement_level === 'recommended') {
    return r.source_ids?.some(id => ['iso45001_2018','iso14001_2026','osha_hazcom','osha_psm','osha_loto','osha_confined','ccps_tpa','hse_competence'].includes(id))
      ? 'international_practice' : 'enterprise_risk_control';
  }
  return r.requirement_level === 'mandatory' ? 'explicit_training_clause' : 'statutory_duty_inference';
};
const add = (r) => rules.push({
  rule_id: `TM3-${String(++n).padStart(3,'0')}`,
  industries: [], roles_any: [], personnel_statuses_any: [], risk_tags_any: [], risk_tags_all: [], risk_tags_none: [], special_work_modes: {},
  requirement_level: 'recommended', frequency: '由企业基于风险确定', drill_frequency: null, minimum_duration: null,
  competence_level: null, assessment_requirement: null, record_requirement: '培训记录与效果评价',
  priority: 'medium', internal_review_required: true, ...r, basis_kind: inferredBasisKind(r)
});
...
const allWorkers = roles.map(role=>role.id);
...
add({roles_any:['fire_manager'],topic_id:'fire_manager',requirement_level:'mandatory',reason:'被指定的消防安全责任人、管理人及专兼职消防管理人员应接受消防安全专门培训。',source_ids:['fire_law','fire_order_61'],training_trigger:'任职前；消防职责、设施或风险变化时',frequency:'专门培训＋持续更新',competence_level:'management',assessment_requirement:'履职知识与现场能力确认',priority:'high'});
add({roles_any:['fire_control_operator'],topic_id:'fire_control_room',requirement_level:'mandatory',reason:'消防控制室值班、操作人员应接受专门培训，并按适用规定核对资格及持证要求。',source_ids:['fire_order_61'],training_trigger:'独立值班或操作前；系统、职责或法规变化时',frequency:'准入/变化触发＋企业持续能力复核',competence_level:'certified',assessment_requirement:'理论、设备实操和报警处置考核',record_requirement:'资格/能力证明、培训与值班授权记录',priority:'high',basis_kind:'statutory_qualification'});
...
add({roles_any:operational,personnel_statuses_any:['new_hire'],risk_tags_none:['hazchem_high_risk_unit'],topic_id:'induction_general',requirement_level:'mandatory',reason:'一般生产经营单位新上岗从业人员应接受与实际岗位相匹配的岗前培训；加工、制造等生产单位实施三级安全培训。',source_ids:['safe_production_law','training_order_3'],training_trigger:'新上岗前',frequency:'上岗前',minimum_duration:'岗前安全培训不少于24学时',competence_level:'practical',assessment_requirement:'培训合格并具备岗位操作和应急能力',priority:'high'});
add({roles_any:operational,personnel_statuses_any:['new_hire'],risk_tags_all:['hazchem_high_risk_unit'],topic_id:'induction_high_risk',requirement_level:'mandatory',reason:'危险化学品等规定范围生产经营单位的新上岗人员适用更高岗前和年度再培训学时。',source_ids:['safe_production_law','training_order_3'],training_trigger:'新上岗前；每年再培训',frequency:'上岗前＋每年再培训',minimum_duration:'岗前不少于72学时；每年再培训不少于20学时',competence_level:'practical',assessment_requirement:'培训考核合格并具备岗位实操、自救互救和应急能力',priority:'high'});
add({roles_any:operational,personnel_statuses_any:['transferred','returned'],topic_id:'job_sop',requirement_level:'mandatory',reason:'转岗或复岗人员应在独立工作前重新确认新岗位风险、操作规程和应急能力。',source_ids:['safe_production_law','training_order_3'],training_trigger:'转岗或复岗前',frequency:'每次转岗/复岗触发',competence_level:'practical',assessment_requirement:'岗位知识与实操确认',priority:'high'});
add({roles_any:operational,personnel_statuses_any:['dispatched','intern'],topic_id:'dispatched_intern',requirement_level:'mandatory',reason:'被派遣劳动者和实习学生应纳入使用单位统一管理，并接受与实际岗位相匹配的安全培训。',source_ids:['safe_production_law','training_order_3'],training_trigger:'进入岗位前；岗位或作业变化时',frequency:'上岗/变化触发',competence_level:'practical',assessment_requirement:'岗位知识与实操确认',priority:'high'});
...
add({roles_any:['maintenance','welding_worker','contractor_worker'],risk_tags_all:['hot_work'],special_work_modes:{hot_work:['perform']},topic_id:'hot_work_performer',requirement_level:'mandatory',reason:'确认本人直接实施焊接与热切割等目录内特种作业后，应完成专门培训、考核取证并掌握2025版焊接切割安全要求。',source_ids:['safe_production_law','special_work_19','gb9448_2025','gb30871_2022'],training_trigger:'从事适用作业前；换证、工艺/设备或作业条件变化时',frequency:'证书有效期6年；届满按令第19号完成培训、考试和换证；企业内部能力另行保持',competence_level:'certified',assessment_requirement:'法定理论和实操考核＋企业现场授权',record_requirement:'有效特种作业操作证、培训与授权记录',priority:'high',basis_kind:'statutory_qualification'});
...
add({roles_any:['maintenance','electrician','instrument_technician','contractor_worker'],risk_tags_any:['electrical_work','temporary_power'],special_work_modes:{electrical_work:['perform'],temporary_power:['perform']},topic_id:'electrical_special',requirement_level:'mandatory',reason:'确认本人直接实施目录内电工作业后，应经专门安全技术培训考核并取得相应操作证。',source_ids:['safe_production_law','special_work_19'],training_trigger:'从事适用电工作业前；换证、作业项目或技术变化时',frequency:'证书有效期6年；届满按令第19号完成培训、考试和换证；企业内部能力另行保持',competence_level:'certified',assessment_requirement:'法定理论和实操考核＋企业授权',record_requirement:'有效操作证、培训与授权记录',priority:'high',basis_kind:'statutory_qualification'});
...
add({roles_any:['maintenance','high_place_worker','contractor_worker'],risk_tags_all:['work_at_height'],special_work_modes:{work_at_height:['perform']},topic_id:'work_at_height',requirement_level:'mandatory',reason:'确认本人直接实施特种作业目录范围内的高处作业后，应取证并掌握现行高处作业与防坠落要求。',source_ids:['safe_production_law','special_work_19','gb3608_2025','gb30871_2022'],training_trigger:'作业前；取证/换证；设备、系统或作业条件变化时',frequency:'证书有效期6年；届满按令第19号完成培训、考试和换证；企业内部能力另行保持',competence_level:'certified',assessment_requirement:'法定考核＋防坠落实操和救援认知',priority:'high',basis_kind:'statutory_qualification'});
...
add({roles_any:allWorkers,topic_id:'fire_all',requirement_level:'mandatory',reason:'单位在岗职工应接受消防安全培训，掌握本岗位火灾危险、防火措施、消防设施、报警、初起火灾处置和逃生。',source_ids:['fire_law','fire_education_109'],training_trigger:'新上岗/新岗位前；消防风险、设施或预案变化时',frequency:'在岗职工每年至少一次；新上岗或进入新岗位前另行培训',drill_frequency:'演练频次单独按单位类型和预案确定，不与培训频次合并',competence_level:'practical',assessment_requirement:'灭火器材或疏散情景实操',priority:'high',basis_kind:'explicit_training_clause'});
add({roles_any:allWorkers,risk_tags_all:['fire_key_unit'],topic_id:'fire_key_all',requirement_level:'mandatory',reason:'消防安全重点单位应对每名员工至少每年进行一次消防安全培训。',source_ids:['fire_order_61'],training_trigger:'确认重点单位属性后纳入年度计划；新上岗/新岗位前另行培训',frequency:'每年至少一次',drill_frequency:'至少每半年组织一次灭火和应急疏散演练',competence_level:'practical',assessment_requirement:'知识与消防器材/疏散实操考核',priority:'high',basis_kind:'explicit_training_clause'});
add({roles_any:allWorkers,risk_tags_all:['public_gathering_place'],topic_id:'fire_public_all',requirement_level:'mandatory',reason:'公众聚集场所员工消防安全培训至少每半年一次，并应掌握组织、引导在场群众疏散的知识和技能。',source_ids:['fire_order_61'],training_trigger:'确认公众聚集场所属性后纳入半年度计划；新上岗/新岗位前另行培训',frequency:'至少每半年一次',drill_frequency:'按重点单位属性及预案要求单独确定演练频次',competence_level:'practical',assessment_requirement:'知识与组织引导疏散实操考核',priority:'high',basis_kind:'explicit_training_clause'});
...
add({roles_any:[...allWorkers,'supervisor'],topic_id:'emergency_general',requirement_level:'mandatory',reason:'生产经营单位应对从业人员进行应急教育和培训，使其具备必要应急知识并掌握风险防范技能和事故应急措施。',source_ids:['safe_production_law','emergency_reg_708'],training_trigger:'上岗前；预案、风险或职责变化时；演练或事件暴露差距时',frequency:'上岗/变化/绩效触发；全国法规未统一规定固定年度培训频次',drill_frequency:'按适用预案管理规定和企业演练计划单独确定',competence_level:'practical',assessment_requirement:'情景考核或演练评价',priority:'high',basis_kind:'explicit_training_clause'});
add({roles_any:['emergency_team'],topic_id:'emergency_team',requirement_level:'mandatory',reason:'被指定的应急救援人员应经培训合格后参加救援，并定期训练。',source_ids:['emergency_reg_708','emergency_plan_88'],training_trigger:'加入队伍前；职责、风险、装备或预案变化时；演练/事件后',frequency:'准入培训＋定期训练',drill_frequency:'按预案和风险组织实战演练',competence_level:'rescue',assessment_requirement:'实战演练与体能/装备使用评价',priority:'high',basis_kind:'explicit_training_clause'});
...
const personnelStatusIds = new Set(personnelStatuses.map(x=>x.id));
for (const r of rules) {
  ...
  for (const id of r.personnel_statuses_any) if (!personnelStatusIds.has(id)) throw new Error(`Unknown personnel status ${id}`);
...
const catalog = {
  ...
  special_work_checks: [
    {risk_id:'hot_work',label:'动火/焊接热切割身份确认',trigger_roles:['maintenance','welding_worker','contractor_worker'],options:[{id:'perform',name:'本人实施'},{id:'affected',name:'仅受影响/在附近工作'},{id:'control',name:'审批、监护或属地管理'},{id:'not_perform',name:'不实施'}]},
    {risk_id:'electrical_work',label:'电工作业身份确认',trigger_roles:['maintenance','electrician','instrument_technician','contractor_worker'],options:[{id:'perform',name:'本人实施'},{id:'affected',name:'仅受影响'},{id:'control',name:'审批或属地管理'},{id:'not_perform',name:'不实施'}]},
    {risk_id:'temporary_power',label:'临时用电安装/接线身份确认',trigger_roles:['maintenance','electrician','instrument_technician','contractor_worker'],options:[{id:'perform',name:'本人实施安装或接线'},{id:'affected',name:'仅使用/受影响'},{id:'control',name:'审批或检查'},{id:'not_perform',name:'不实施'}]},
    {risk_id:'work_at_height',label:'高处特种作业身份确认',trigger_roles:['maintenance','high_place_worker','contractor_worker'],options:[{id:'perform',name:'本人实施'},{id:'affected',name:'仅受影响/配合'},{id:'control',name:'审批、监护或属地管理'},{id:'not_perform',name:'不实施'}]}
  ],
  industries, roles, personnel_statuses: personnelStatuses, risk_tags: riskTags, topics, sources, rules
};
fs.writeFileSync(out, JSON.stringify(catalog, null, 2) + '\n');
console.log(`Wrote ${out}`);
console.log(JSON.stringify({industries:industries.length,roles:roles.length,personnel_statuses:personnelStatuses.length,risk_tags:riskTags.length,topics:topics.length,sources:sources.length,rules:rules.length}));
